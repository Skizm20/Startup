...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Logo element
    const logoLink = document.querySelector('.logo') || document.getElementById('main-logo');
    ...
    const updateLogoLink = (isLoggedIn) => {
        if (logoLink) {
            const newHref = isLoggedIn ? 'explore.html' : 'index.html';
            logoLink.href = newHref;
            console.log(`Logo link updated to: ${newHref} (logged in: ${isLoggedIn})`);
        } else {
    ...
    // ---
    // LOGO CLICK HANDLER: Dynamic navigation based on current auth state
    // ---
    if (logoLink) {
        logoLink.addEventListener('click', (e) => {
            ...
            if (isLoggedIn) {
                console.log('Logo clicked - Navigating to explore.html (logged in)');
                window.location.href = 'explore.html';
            } else {
                console.log('Logo clicked - Navigating to index.html (logged out)');
    ...
    // --- 11. PRICING PAGE DYNAMIC FUNCTIONALITY ---
    ...
    if (billingToggle && pricingCards.length > 0) {
        ...
        function updatePricing(isYearly) {
            pricingCards.forEach(card => {
                ...
                const monthlyUSD = parseFloat(priceElement.dataset.monthly);
                const yearlyUSD = parseFloat(priceElement.dataset.yearly);
                
                // Skip free plan
                if (monthlyUSD === 0) return;
                
                const baseAmount = isYearly ? yearlyUSD : monthlyUSD;
                const converted = convertCurrency(baseAmount, 'USD', currentCurrency);
                const formatted = formatCurrency(converted, currentCurrency, false);
                ...
                if (amountElement && currencyElement) {
                    ...
                    setTimeout(() => {
                        amountElement.textContent = formatted.amount;
                        currencyElement.textContent = formatted.symbol;
                ...
                // Update period text
                if (periodText) {
                    periodText.textContent = isYearly ? 'year' : 'month';
                }
                
                // Show/hide savings text
                if (savingsText && savingsAmount) {
                    if (isYearly) {
                        const monthlyCost = monthlyUSD * 12;
                        const savings = convertCurrency(monthlyCost - yearlyUSD, 'USD', currentCurrency);
                        const formattedSavings = formatCurrency(savings, currentCurrency);
    ...
    // --- 13. 3D TILT EFFECT + CURSOR GLOW FOR PRICING CARDS ---
    if (pricingCards.length > 0) {
        pricingCards.forEach(card => {
            card.addEventListener('mousemove', (e) => {
                ...
                const glowX = x - 150; // Center the 300px glow
                const glowY = y - 150;
    ...
    // --- 14. PARALLAX EFFECT ON SCROLL FOR PRICING SECTION ---
    ...
    if (pricingHeader) {
        window.addEventListener('scroll', () => {
            ...
            if (pricingHeader) {
                pricingHeader.style.transform = `translateY(${rate}px)`;
            }
        });
    }

    // --- 15. ANIMATE FEATURES ON CARD HOVER ---
    if (pricingCards.length > 0) {
        pricingCards.forEach(card => {
    ...
    planButtons.forEach(button => {
        button.addEventListener('mousemove', (e) => {
            ...
            button.style.transform = `translate(${moveX}px, ${moveY}px) translateY(-3px) scale(1.05)`;
        });
    ...
    // --- 17. SMOOTH SCROLL REVEAL FOR FAQ ITEMS ---
    ...
    // --- 18. ANIMATE ALL CARDS ON SCROLL ---
    ...
    // Apply to all startup cards and pricing cards
    const cardsToAnimate = document.querySelectorAll('.startup-card, .pricing-card, .input-group, .modal');
    ...
    // --- 19. SMOOTH BUTTON CLICK ANIMATION ---
    ...
    // --- 21. AUTO-SELECT ROLE BASED ON URL PARAMETER (SIGNUP PAGE) ---
    const urlParams = new URLSearchParams(window.location.search);
    const planParam = urlParams.get('plan');
    
    if (planParam && (planParam === 'founder' || planParam === 'investor')) {
        const roleRadio = document.getElementById(`role-${planParam}`);
        if (roleRadio) {
            roleRadio.checked = true;
            
            // Add visual feedback with animation
            const roleLabel = roleRadio.nextElementSibling;
            if (roleLabel) {
                roleLabel.style.animation = 'pulse 0.5s ease';
                setTimeout(() => {
                    roleLabel.style.animation = '';
                }, 500);
            }
            
            console.log(`Auto-selected role: ${planParam}`);
        }
    }

    // --- 22. ANIMATED COUNTER FOR HERO STATS (HOMEPAGE) ---
